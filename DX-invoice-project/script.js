...
function update_totals() {
  ...
  grand_total = sub_total + parseFloat($$("other_charges").value) - parseFloat($$("discount").value);
  $$("grand_total_D").innerHTML = grand_total + ' FCFA';
}
...
function add_a_line_item() {
  ...
  input.min = 1;
  input.placeholder = "Cost per Product";
  input.value = "#";
  order_items.appendChild(input);
  ...
  input.min = 1;
  input.placeholder = "No. of Purchase";
  input.value = "#";
  order_items.appendChild(input);
...
function set_defaults() {
  ...
  //set invoice number
  invoice_number =
    "" + now.getFullYear() + now.mm + now.dd + now.hh + "-" + now.getMinutes();
